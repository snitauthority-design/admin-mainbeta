...
export const StoreHeader: React.FC<StoreHeaderProps> = (props) => {
  ...
  // Expose cart drawer open function to parent
  const openCartDrawerRef = useRef(() => setIsCartDrawerOpen(true));
  useEffect(() => {
    if (onCartOpenRef) {
      onCartOpenRef(openCartDrawerRef.current);
    }
  }, [onCartOpenRef]);
  ...
  const categoryMenuRef = useRef<HTMLDivElement>(null!) as React.RefObject<HTMLDivElement>;
    ...
    <>
      ...
      <header className="store-header w-full bg-white shadow-sm sticky top-0 z-50 transition-colors duration-300">
        ...
        <DesktopHeaderBar
          ...
          childCategories={safeChildCategories}
        />
      </header>
