      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative">
        ...
        <div className="p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading} className="w-full py-3 rounded-xl font-bold text-white disabled:opacity-60 transition-all hover:shadow-lg active:scale-[0.98]" style={{ background: 'linear-gradient(135deg, #FB923C 0%, #F97316 60%, #EA580C 100%)', boxShadow: '0 8px 24px rgba(249,115,22,0.3)' }}>{loading ? 'Please wait...' : isLogin ? 'Login' : 'Register'}</button>
            {error && <p className="text-sm text-red-500 text-center">{error}</p>}
          </form>
          {onGoogleLogin && <div className="mt-6">
            <div className="relative flex items-center justify-center mb-3"><span className="px-3 text-xs text-gray-400 bg-white relative z-10">OR</span><div className="absolute left-0 right-0 h-px bg-gray-200"/></div>
            <button type="button" onClick={handleGoogle} disabled={gLoading} className="w-full flex items-center justify-center gap-2 border border-gray-200 rounded-xl py-3 font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-60 transition-all"><GoogleIcon/>{gLoading ? 'Connecting...' : 'Continue with Google'}</button>
          </div>}
          <div className="mt-6 text-center text-sm text-gray-600">{isLogin ? "Don't have an account? " : "Already have an account? "}<button onClick={() => setIsLogin(!isLogin)} className="font-bold hover:underline" style={{ color: '#0EA5E9' }}>{isLogin ? 'Sign Up' : 'Login'}</button></div>
        </div>
      </div>
    </div>
  );
