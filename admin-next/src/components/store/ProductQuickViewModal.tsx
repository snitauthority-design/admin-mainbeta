    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xl p-4">
      <div className="bg-white/95 backdrop-blur-2xl border border-white/50 rounded-3xl shadow-2xl max-w-4xl w-full overflow-hidden grid grid-cols-1 lg:grid-cols-2">
        ...
        <div className="p-6 lg:p-10 space-y-4">
          ...
          {p.colors?.length && <div><p className="text-xs font-semibold text-gray-500 uppercase mb-2">Color</p><div className="flex flex-wrap gap-2">{p.colors.map(c => <button key={c} type="button" onClick={() => setColor(c)} className={`w-8 h-8 rounded-full border-2 ${color === c ? 'border-sky-500' : 'border-transparent'} shadow-sm`} style={{ backgroundColor: c }}/>)}</div></div>}
          {p.sizes?.length && <div><p className="text-xs font-semibold text-gray-500 uppercase mb-2">Size</p><div className="flex flex-wrap gap-2">{p.sizes.map(s => <button key={s} type="button" onClick={() => setSize(s)} className={`px-3 py-1.5 rounded-xl border text-sm font-semibold ${size === s ? 'border-sky-500 text-sky-600 bg-sky-50' : 'border-gray-200 text-gray-600'}`}>{s}</button>)}</div></div>}
          ...
          <div className="flex flex-col gap-3">
            <button type="button" onClick={() => onCompleteOrder(p, qty, variant)} className="w-full backdrop-blur-md border border-white/30 text-white py-3 rounded-2xl font-bold text-base shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all" style={{ background: 'linear-gradient(135deg, #FB923C 0%, #F97316 60%, #EA580C 100%)' }}>Complete Order</button>
            <button type="button" onClick={() => onViewDetails?.(p)} className="w-full bg-white/80 backdrop-blur-md py-3 rounded-2xl border border-gray-200 text-gray-700 font-semibold hover:border-sky-400 hover:bg-white transition-all">View Full Details</button>
          </div>
        </div>
      </div>
    </div>
