...
export function useCart({ user, products, tenantId }: Options) {
  ...
  // Skip the next server save after loading cart from server (prevents fetch→save loop)
  ...
  useEffect(() => {
    if (!user?.id) return;
    let cancel = false;
    (async () => { try { const r = await DataService.get('cart', [], user.id); if (!cancel && Array.isArray(r)) { skipNextServerSaveRef.current = true; setCart(r); } } catch {} })();
    return () => { cancel = true; };
  }, [user?.id]);

  useEffect(() => {
    if (!user?.id) return;
    if (skipNextServerSaveRef.current) { skipNextServerSaveRef.current = false; return; }
    DataService.save('cart', cart, user.id).catch(() => {});
  }, [cart, user?.id]);
